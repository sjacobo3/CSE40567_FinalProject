...
def aes_encrypt_file(plaintext, key):
    # convert plaintext to list and add padding so it is compatible with our functions
    ...
    ciphertext = []
    prev = iv
    # go through each 16-byte block in the plaintext
    for i in range(0, len(plaintext), 16):
        # get current block
        block = plaintext[i:i+16]
        # process block using AES encryption
        xored = xor_bytes(block, prev)
        encrypted = encrypt(xored, key)
        ciphertext.extend(encrypted)
        prev = encrypted
    # convert result to bytes and return it
    return bytes(iv + ciphertext)
def aes_decrypt_file(ciphertext, key):
    # convert ciphertext to list so it is compatible with our functions
    ...
    plaintext = []
    prev = iv
    # go through each 16-byte block in the ciphertext
    for i in range(0, len(ciphertext), 16):
        # get current block
        block = ciphertext[i:i+16]
        # process block using AES decryption
        decrypted = decrypt(block, key)
        xored = xor_bytes(decrypted, prev)
        plaintext.extend(xored)
        prev = block
    # remove padding, convert result to bytes, and return it
